...
class UpBlock_attention(nn.Module):
    def __init__(self, in_channels, out_channels, nb_Conv, activation='ReLU'):
        ...
        self.coatt = GAM_Attention(in_channels//2, in_channels//2)
        self.nConvs = _make_nConv(in_channels, out_channels, nb_Conv, activation)

    def forward(self, x, skip_x):
        up = self.up(x)
        skip_x_att = self.coatt(skip_x)
        x = torch.cat([skip_x_att, up], dim=1)  # dim 1 is the channel dimension
        return self.nConvs(x)

class GAMUCTransNet(nn.Module):
    def __init__(self, config,n_channels=3, n_classes=1,img_size=224,vis=False):
        ...
        # self.gam4 = GAM_Attention(in_channels*4, in_channels*4)
        self.up3 = UpBlock_attention(in_channels*8, in_channels*2, nb_Conv=2)
        # self.gam3 = GAM_Attention(in_channels*2, in_channels*2)
        self.up2 = UpBlock_attention(in_channels*4, in_channels, nb_Conv=2)
        # self.gam2 = GAM_Attention(in_channels, in_channels)
        self.up1 = UpBlock_attention(in_channels*2, in_channels, nb_Conv=2)
        # self.gam1 = GAM_Attention(in_channels, in_channels)
